    ...
    def face_recognition_with_gpu(self, img, quiet=True, size=(96, 96),
        ...
        for i in range(faces.shape[2]):
            ...
            if confidence > 0.5:
                ...
                label, end2, pred_score, face_image = self.predict_face_emotion(
                    face_image, size, processing, deep_index=deep_index, deep_model=deep_model)
...
#                 end2 = time.time() - start
                dic_face[i] = self.__set_dict_information__(
                    ...
                    y1=y1,
                    emotion=label,
                    confidence=int(pred_score),
                    time_predict=end2,
                    face_encode="data:image/jpeg;base64," +
                    encode_image(face_image))
            else:
                continue
        if not dic_face:
...
#         print(img.shape)
    ...
    def __handle_label(self, label: str, score: float):
        return "Unknown" if score < 55 else label

    def face_recognition_with_face_lib(self, image, quiet=True,
                                       size=(96, 96), processing=0,
                                       deep_model=True, deep_index=0):
        #         print("use lib")
        """
        ...
        if no_of_faces > 0:
            ...
            for i, coordinate in enumerate(faces_coors):
                ...
                label, end2, score, face_image = self.predict_face_emotion(
                    face_image, size, processing, deep_index=deep_index, deep_model=deep_model)
...
#                 print(label)
    ...
    def predict_face_emotion(self, face_image, size: tuple,
                             processing: int, deep_model=True, deep_index=0):
        """
        ...
        if not deep_model:
            ...
            pred, score, end2 = self.enmodel.deep_predict(
                deep_index, face_image)
        # print("Predicted in ", end2)
    #             print(f'Predict in {time.time()-start}s')
        label = self.labels[pred[0]]
#         print(label)
        # end2 = time.time() - start
        # face_image = face_images if processing else face_image
        return label, end2, score, face_images
    ...
    def processing_image(
        ...
        try:
            if gpu == 1:
                face_image, dic_face = self.face_recognition_with_gpu(
                    img, quiet, size, processing, deep_model=deep_model, deep_index=deep_index)
            elif gpu == 0:
                face_image, dic_face = self.face_recognition_with_cpu(
                    img, quiet, size, processing, deep_model=deep_model, deep_index=deep_index)
            else:
                face_image, dic_face = self.face_recognition_with_face_lib(
                    img, quiet, size, processing, deep_model=deep_model, deep_index=deep_index)
    ...
    def detect_emotion_with_image(self, img_path, show=True, gpu=1,
        ...
        face, dict_face = self.processing_image(img_path, gpu, quiet,
                                                size=size, processing=processing,
                                                deep_index=deep_index, deep_model=deep_model)
...
#             print(full_path)
            ...
            face, label = self.detect_emotion_with_image(
                full_path, gpu=gpu, show=show, save=save, quiet=quiet, size=size)
#             print(label)
            ...
            #     faces = face_cascade.detectMultiScale()
                ...
                try:
                    face_emotion, end, end2, label = self.processing_image(
                        frame, gpu, quiet, resize=False, size=size)
#                     print(end, end2)
                    frame = face_emotion
